...
export class UsersService {
  ...
  async updateRole(id: string, role: Role, actor: AuthenticatedUser): Promise<UserDocument> {
    ...
    const user = await this.userModel.findByIdAndUpdate(id, { role }, { returnDocument: 'after' }).exec();
    if (!user) {
      throw new NotFoundException('User not found');
    }
    return user;
  ...
  async updateStatus(id: string, isActive: boolean, actor: AuthenticatedUser): Promise<UserDocument> {
    if (id === actor.userId) {
      throw new BadRequestException('You cannot change your own account status');
    }
    const user = await this.userModel.findByIdAndUpdate(id, { isActive }, { returnDocument: 'after' }).exec();
    if (!user) {
      throw new NotFoundException('User not found');
    }
    return user;
