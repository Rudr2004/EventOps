...
export class TasksService {
  ...
  async findAll(query: QueryTasksDto, actor: AuthenticatedUser): Promise<PaginatedResult<TaskDocument>> {
    ...
    if (actor.role === Role.EVENT_MANAGER) {
      ...
      rawFilter.event = { $in: ownedEventIds };
    } else if (actor.role === Role.OPERATIONS_MEMBER) {
      rawFilter.assignee = new Types.ObjectId(actor.userId);
    }
  ...
  async findByIdScoped(id: string, actor: AuthenticatedUser): Promise<TaskDocument> {
    ...
    if (actor.role === Role.EVENT_MANAGER) {
      const ownedEventIds = await this.eventsService.findOwnedEventIds(actor.userId);
      const owned = ownedEventIds.some((id) => id.toString() === task.event.toString());
      if (!owned) {
        throw new NotFoundException('Task not found');
      }
    }
