...
export class EventsService {
  ...
  async findByIdScoped(id: string, actor: AuthenticatedUser): Promise<EventDocument> {
  ...
  /**
   * Ids of events owned by this Event Manager, for scoping Tasks/Incidents
   * queries to "tasks/incidents on events I own". Only meaningful for the
   * Event Manager role — callers should only invoke this for that role.
   */
  async findOwnedEventIds(ownerId: string): Promise<Types.ObjectId[]> {
    const owned = await this.eventModel
      .find({ owner: new Types.ObjectId(ownerId) })
      .select('_id')
      .exec();
    return owned.map((e) => e._id as Types.ObjectId);
  }
