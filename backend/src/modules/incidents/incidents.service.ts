...
export class IncidentsService {
  ...
  async create(eventId: string, dto: CreateIncidentDto, actor: AuthenticatedUser): Promise<IncidentDocument> {
    ...
    if (event.status === EventStatus.COMPLETED || event.status === EventStatus.ARCHIVED) {
      throw new BadRequestException(
        `Cannot create incidents on an event that is ${event.status}`,
      );
    }
  ...
  async findAll(
    ...
    } else if (actor.role === Role.OPERATIONS_MEMBER) {
      filter.assignee = new Types.ObjectId(actor.userId);
    }
  ...
  async findByIdScoped(id: string, actor: AuthenticatedUser): Promise<IncidentDocument> {
    ...
    if (actor.role === Role.EVENT_MANAGER) {
      const ownedEventIds = await this.eventsService.findOwnedEventIds(actor.userId);
      const owned = ownedEventIds.some((id) => id.toString() === incident.event.toString());
      if (!owned) {
        throw new NotFoundException('Incident not found');
      }
    }
  ...
  async getTimeline(id: string, actor: AuthenticatedUser): Promise<IncidentDocument> {
    return this.findByIdScoped(id, actor);
  }
