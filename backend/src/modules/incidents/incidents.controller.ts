...
export class IncidentsController {
  ...
  @Get('incidents')
  async findAll(@Query() query: QueryIncidentsDto, @CurrentUser() user: AuthenticatedUser) {
    const result = await this.incidentsService.findAll(query, user);
    return {
  ...
  async findOne(@Param('id') id: string, @CurrentUser() user: AuthenticatedUser) {
    const incident = await this.incidentsService.findByIdScoped(id, user);
    return toIncidentResponse(incident);
  }
  ...
  async getTimeline(@Param('id') id: string, @CurrentUser() user: AuthenticatedUser) {
    const incident = await this.incidentsService.getTimeline(id, user);
