...
describe('Events (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const opsEmail = uniqueEmail('ops');
    createdUserEmails.push(opsEmail);
    await request(app.getHttpServer())
      .post('/api/v1/auth/register')
      .send({ name: 'Ops User', email: opsEmail, password: 'StrongP@ssw0rd' });
    await connection
      .collection('users')
      .updateOne({ email: opsEmail }, { $set: { role: 'operations_member' } });
    const opsLogin = await request(app.getHttpServer())
      .post('/api/v1/auth/login')
      .send({ email: opsEmail, password: 'StrongP@ssw0rd' });
    opsToken = (opsLogin.body.data as AuthResult).accessToken;

    const otherManagerEmail = uniqueEmail('other-manager');
    createdUserEmails.push(otherManagerEmail);
    const otherManagerRes = await request(app.getHttpServer())
      .post('/api/v1/auth/register')
      .send({ name: 'Other Manager User', email: otherManagerEmail, password: 'StrongP@ssw0rd' });
    otherManagerId = (otherManagerRes.body.data as AuthResult).user.id;
    await connection
      .collection('users')
    ...
    otherManagerToken = (otherManagerLogin.body.data as AuthResult).accessToken;
  });
  ...
  describe('row-level read scoping', () => {
    let managerOwnedEventId: string;
    let otherManagerOwnedEventId: string;

    beforeAll(async () => {
      const managerEventRes = await request(app.getHttpServer())
        .post('/api/v1/events')
        .set('Authorization', `Bearer ${managerToken}`)
        .send({
          name: 'Scoping - Manager Owned',
          venue: 'Hall D',
          startDate: '2027-01-01T09:00:00.000Z',
          endDate: '2027-01-02T09:00:00.000Z',
        })
        .expect(201);
      managerOwnedEventId = managerEventRes.body.data.id;
      createdEventIds.push(managerOwnedEventId);

      const otherManagerEventRes = await request(app.getHttpServer())
        .post('/api/v1/events')
        .set('Authorization', `Bearer ${otherManagerToken}`)
        .send({
          name: 'Scoping - Other Manager Owned',
          venue: 'Hall E',
          startDate: '2027-01-03T09:00:00.000Z',
          endDate: '2027-01-04T09:00:00.000Z',
        })
        .expect(201);
      otherManagerOwnedEventId = otherManagerEventRes.body.data.id;
      createdEventIds.push(otherManagerOwnedEventId);
    });

    it('lets an Event Manager see only their own events in the list', async () => {
      const res = await request(app.getHttpServer())
      ...
      expect(ids).toContain(managerOwnedEventId);
      expect(ids).not.toContain(otherManagerOwnedEventId);
    });

    it('lets an Event Manager fetch their own event by id', async () => {
      const res = await request(app.getHttpServer())
        .get(`/api/v1/events/${managerOwnedEventId}`)
        .set('Authorization', `Bearer ${managerToken}`)
        .expect(200);

      expect(res.body.data.id).toBe(managerOwnedEventId);
    });

    it('hides another manager\'s event from an Event Manager (404)', async () => {
      await request(app.getHttpServer())
        .get(`/api/v1/events/${otherManagerOwnedEventId}`)
        .set('Authorization', `Bearer ${managerToken}`)
        .expect(404);
    });

    it('overrides an explicit ?owner= query param for an Event Manager', async () => {
      const res = await request(app.getHttpServer())
        .get(`/api/v1/events?owner=${otherManagerId}&limit=100`)
        .set('Authorization', `Bearer ${managerToken}`)
        .expect(200);
    ...
    it('lets an Operations Member see all events in the list', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/v1/events?limit=100')
      ...
      const ids = res.body.data.items.map((e: { id: string }) => e.id);
      expect(ids).toContain(managerOwnedEventId);
      expect(ids).toContain(otherManagerOwnedEventId);
    });

    it('lets a Viewer fetch any event by id', async () => {
      await request(app.getHttpServer())
        .get(`/api/v1/events/${otherManagerOwnedEventId}`)
        .set('Authorization', `Bearer ${viewerToken}`)
        .expect(200);
    ...
    it('lets a Viewer see all events in the list regardless of owner', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/v1/events?limit=100')
        .set('Authorization', `Bearer ${viewerToken}`)
        .expect(200);
