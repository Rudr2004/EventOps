...
describe('Tasks (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const otherManager = await registerWithRole('tasks-other-manager', 'event_manager');
  ...
  describe('row-level read scoping', () => {
    it('hides tasks on events another Event Manager does not own', async () => {
      const taskId = createdTaskIds[0];
      const res = await request(app.getHttpServer())
    ...
    it('hides a task from an Event Manager who does not own its event (404)', async () => {
      const taskId = createdTaskIds[0];
      await request(app.getHttpServer())
    ...
    it('scopes an Operations Member to only their assigned tasks in the list', async () => {
      const taskId = createdTaskIds[0];
      const res = await request(app.getHttpServer())
        .get('/api/v1/tasks?limit=100')
        .set('Authorization', `Bearer ${opsToken}`)
      ...
      const otherRes = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${otherOpsToken}`)
        .expect(200);
      const otherIds = otherRes.body.data.items.map((t: { id: string }) => t.id);
    ...
    it('lets a Viewer see all tasks in the list', async () => {
      const taskId = createdTaskIds[0];
      const res = await request(app.getHttpServer())
        .get('/api/v1/tasks?limit=100')
        .set('Authorization', `Bearer ${viewerToken}`)
