...
describe('Incidents (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const admin = await registerWithRole('incidents-admin', 'admin');
    adminToken = admin.accessToken;

    const otherManager = await registerWithRole('incidents-other-manager', 'event_manager');
  ...
  it('allows an Operations Member to create an incident (spec: report as it happens)', async () => {
    const res = await request(app.getHttpServer())
      ...
      .send({ title: 'Spotted a tripping hazard', severity: 'low' })
      .expect(201);
  ...
  it('creates a critical incident assigned to an Operations Member (happy path)', async () => {
    ...
    criticalIncidentId = res.body.data.id;
    createdIncidentIds.push(criticalIncidentId);
  });
  ...
  it('allows the assigned Operations Member to move the incident to Investigating', async () => {
    const incidentId = criticalIncidentId;
    const res = await request(app.getHttpServer())
  ...
  it('rejects creating an incident on a Completed event', async () => {
    ...
    await request(app.getHttpServer())
      .post(`/api/v1/events/${completedEventId}/incidents`)
      .set('Authorization', `Bearer ${managerToken}`)
      .send({ title: 'Too late', severity: 'low' })
      .expect(400);
  });

  describe('row-level read scoping', () => {
    it('hides incidents on events another Event Manager does not own', async () => {
      const incidentId = criticalIncidentId;
      const res = await request(app.getHttpServer())
    ...
    it('hides an incident from an Event Manager who does not own its event (404)', async () => {
      const incidentId = criticalIncidentId;
      await request(app.getHttpServer())
    ...
    it('scopes an Operations Member to only their assigned incidents in the list', async () => {
      const incidentId = criticalIncidentId;
      const res = await request(app.getHttpServer())
        .get('/api/v1/incidents?limit=100')
        .set('Authorization', `Bearer ${opsToken}`)
      ...
      const otherRes = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${otherOpsToken}`)
        .expect(200);
      const otherIds = otherRes.body.data.items.map((i: { id: string }) => i.id);
      expect(otherIds).not.toContain(incidentId);
    });
    ...
    it('lets a Viewer see all incidents in the list', async () => {
      const incidentId = criticalIncidentId;
      const res = await request(app.getHttpServer())
        .get('/api/v1/incidents?limit=100')
        .set('Authorization', `Bearer ${viewerToken}`)
