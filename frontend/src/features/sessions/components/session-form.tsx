...
/** Local (not UTC) "now", formatted for a datetime-local input's `min` attribute. */
function nowForDateTimeLocalMin(): string {
  const now = new Date();
  now.setSeconds(0, 0);
  ...
  return (
    <form className="event-form" onSubmit={handleSubmit}>
      ...
      <FormField
        ...
        min={sessionId ? undefined : nowForDateTimeLocalMin()}
        required
      />
      <FormField
        id="session-end"
